...
| Package                           | Description                                                     |
| --------------------------------- |:----------------------------------------------------------------|
| model                             |  The ``model`` package holds the Entities with the  Business rules. As described in the article described by Uncle Bob ([click here](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html#entities)) the Entities contains the ``most general high-level rules``. For example, the [Customer](https://github.com/ads1986/origin-backend-take-home-assignment/blob/master/src/main/java/com/origin/financial/domain/model/Customer.java) class has the fields which data is stored and the methods with all the rules related to the customer. We can see this in the ``hasAgeAboveSixty()`` method, that will return true if the customer is more than 60 years old.  Rather than repeat the same code throughout the application, we respect the principles [DRY (Don’t Repeat Yourself)](https://pt.wikipedia.org/wiki/Don%27t_repeat_yourself) and [Single Responsability](https://www.baeldung.com/solid-principles#s) centralizing the rule in a single point. |
