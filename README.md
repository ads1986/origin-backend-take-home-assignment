...
| Package                           | Description                                                     |
| --------------------------------- |:----------------------------------------------------------------|
| model                             |  The ``model`` package holds the Entities with the  Business rules. As described in the Uncle Bob's ([click here](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html#entities)) article,  the Entities contains the ``most general high-level rules``. For example, the [Customer](https://github.com/ads1986/origin-backend-take-home-assignment/blob/master/src/main/java/com/origin/financial/domain/model/Customer.java) class has the fields that store  the data and the methods with all the rules related to the customer. We can see this in the ``hasAgeAboveSixty()`` method, that will return true if the customer is more than 60 years old.  Rather than repeat the same code throughout the application, we respect the principles [DRY (Don’t Repeat Yourself)](https://pt.wikipedia.org/wiki/Don%27t_repeat_yourself) and [Single Responsability](https://www.baeldung.com/solid-principles#s) centralizing the rule in a single point. |
| usecase                           |  The ``usecase`` package holds the classes with application specific business rules, in other words, the classes here shows "what the application do". For example, the class [CalculateHomeInsuranceImpl](https://github.com/ads1986/origin-backend-take-home-assignment/blob/master/src/main/java/com/origin/financial/domain/usecase/impl/CalculateHomeInsuranceImpl.java) has the rules to process an insurance for a Home and this funcionality is exposed by the [CalculateHomeInsurance](https://github.com/ads1986/origin-backend-take-home-assignment/blob/master/src/main/java/com/origin/financial/domain/usecase/CalculateHomeInsurance.java) interface, following the [Dependency Inversion](https://www.baeldung.com/solid-principles#d) principle. |
...
| Package                           | Class                                                            |
| --------------------------------- |:----------------------------------------------------------------|
| controller                        | The ``controller`` package holds the classes we use to expose functionality through a REST API. For example, the class [InsuranceController](https://github.com/ads1986/origin-backend-take-home-assignment/blob/master/src/main/java/com/origin/financial/presentation/controller/InsuranceController.java) exposes the API ``[POST] /insurance/profile``. |
...
| Appication                        | Class                                                            |
| --------------------------------- |:----------------------------------------------------------------|
| config                            | For exemple, we have the class [CustomResponseEntityExceptionHandler](https://github.com/ads1986/origin-backend-take-home-assignment/blob/master/src/main/java/com/origin/financial/config/CustomResponseEntityExceptionHandler.java) that handle exceptions thrown by the API, for example, when validating mandatory fields, expected values e etc. |
...
To validate the API endpoint, I provided a Postman project in the endpoints folder. See below : 
